    ...
    protected void RunInspector()
    {
        ...
        if (OpenNISettings.m_useSkeleton)
        {
            ...
            if (m_userAndSkeletonControlFoldout && OpenNISettings.UserSkeletonValid)
            {
                ...
                {
                    int userID = users[i];
                    Vector3 center = OpenNISettings.UserGenerator.GetUserCenterOfMass(userID);
                    ...
                    EditorGUILayout.LabelField("is calibrated:",
                        (OpenNISettings.UserGenerator.Skeleton.IsCalibrated(userID)) ? "Yes" : "No");

                    //OpenNISettings.UserGenerator.Skeleton.GetSkeletonJointPosition(userID, SkeletonJoint.Head).
                    //SkeletonJoint

                    SkeletonCapability skelCap = OpenNISettings.UserGenerator.Skeleton;
                    string skelCapInfo = "not null";
                    if (skelCap == null) skelCapInfo = "null";
                    EditorGUILayout.LabelField("Skeleton Capability:", skelCapInfo);

                    if(skelCap != null)
                    {
                        foreach (SkeletonJoint joint in System.Enum.GetValues(typeof(SkeletonJoint)))
                        {
                            string cStr = "not tracked";

                            try
                            {
                                SkeletonJointPosition skelPos = OpenNISettings.UserGenerator.Skeleton.GetSkeletonJointPosition(userID, joint);
                                cStr = skelPos.Confidence.ToString();
                            }
                            catch (System.Exception ex)
                            {  
                            }

                            EditorGUILayout.LabelField("" + joint, cStr);
                        }
                    }
