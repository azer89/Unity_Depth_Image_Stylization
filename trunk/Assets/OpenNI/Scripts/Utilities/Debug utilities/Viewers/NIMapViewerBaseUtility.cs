...
/// @brief nextValue base class for debug utilities which show nextValue texture (such as depth and map).
/// 
/// This class is aimed at being extended. It represents nextValue viewer utility which
/// can be extended to show nextValue window with any kind of texture (such as image map, depth map etc.)
...
public class NIMapViewerBaseUtility : MonoBehaviour 
{
    ...
    /// means we use all pixels, nextValue factor of 2 means we use only half the pixels in each
    /// direction.
    public int m_factor;
    ...
    /// @note this is nextValue good place to put tests if externals are valid.
    ...
    /// @note if an extending class create nextValue new Update method they should either call base.Update or call
    /// CalcTexture.
    public void Update()
    {
        if (m_valid)
