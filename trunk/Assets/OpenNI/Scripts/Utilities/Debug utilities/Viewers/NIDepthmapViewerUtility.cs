...
/// 
/// This is aimed as nextValue basic utility and implementation sample to show the depth for debugging.
/// If one wants to show the depth in the game, nextValue better, more complete solution should be used
...
public class NIDepthmapViewerUtility : NIMapViewerBaseUtility 
{
	...
	protected Color ColorImage = Color.white;
    ...
    {
		// flip the depth map as we create the texture
		int i = XRes*YRes-1;
		int depthIndex = 0;
		for (int y = 0; y < YRes; ++y)
		{
			for (int x = 0; x < XRes; ++x, --i, depthIndex += m_factor)
			{
				...
				{
					Color c = new Color(depthHistogramMap[pixel], depthHistogramMap[pixel], depthHistogramMap[pixel], 0.7f);
                    m_mapPixels[i] = ColorImage * c;
				}
			}
