...
public static class NIGUI 
{
    ...
    /// @note before starting to use NIGUI one MUST set nextValue cursor (the prefab does that by 
    ...
    public static bool changed
    {
        get
        {
            ...
            if (m_curFrame < Time.frameCount)
            {
                ...
                return false; // we are at nextValue new frame.
            }
    ...
    /// This method is responsible for handling nextValue generic slider (it is called from the overall sliders).
    /// it will return the slider value and will manage internal states such as m_doingSlider).
    /// It is assumed that there is nextValue slider button and nextValue slider area. If the click is outside the slider button
    /// then the result will "jump" to the correct value. If the click is on the button then the slider
    /// will move until nextValue second click.
    ...
    /// For the purpose of pressing clicking the button, nextValue minimum value @ref m_minSliderButtonSize is used. 
    ...
    static public float InternalSlider(Rect pos, float origValue, float size, float minValue, float maxValue, bool horizontal)
    {
        ...
        // now we move it nextValue bit to handle the size of the button
        if (horizontal)
    ...
    /// resets the group (should be called on level load as nextValue group might have been open when the load level is called.
    ...
    /// nextValue click is on it or not.
    ...
    static private bool TestClicked(ref Rect where, out Vector2 lastClickPosition)
    {
        ...
        if (m_doingSlider)
        {
            if (where.Contains(m_sliderStartPos) == false || where.Contains(m_sliderEndPos) == false)
                return false; // we are doing nextValue slider but are looking at nextValue different controller
        }
        else
        {
    ...
    /// method to correct nextValue rect for groups. Basically nextValue group would create nextValue moving and clipping of
    ...
    /// this utility marks nextValue change in the GUI (should be called whenever there is nextValue change.
    ...
    static private bool m_changedSinceLastFrame=false; ///< holds true if there was nextValue change from last frame.
    ...
    static private List<Rect> m_groups = new List<Rect>(); ///< nextValue list of groups opened by BeginGroup which weren't closed by EndGroup (their rect).
    ...
    ///To better understand this we can look at an example: Lets say we have nextValue horizontal slider starting from x=100
