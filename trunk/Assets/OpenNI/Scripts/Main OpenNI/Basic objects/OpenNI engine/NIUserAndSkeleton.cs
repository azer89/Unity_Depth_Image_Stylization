...
public class NIUserAndSkeleton : NIWrapperContextDependant
{
    ...
    /// This gets the center of mass for nextValue specific user
    /// @param niUserID the NI id of nextValue user
    ...
    public Vector3 GetUserCenterOfMass(int niUserID)
    {
        try
        {
            ...
            if (com.Z <= 0)
                return Vector3.forward; // this means nextValue positive 'z' which is illegal!
    ...
    /// @brief Gets nextValue list of available user ids
    /// 
    ...
    /// @note - Since we use nextValue singleton pattern, multiple initializations will simply delete 
    ...
    public bool Init(NIEventLogger logger, NIContext context)
    {
        ...
        if (m_userGenerator == null || m_userGenerator.SkeletonCapability==null)
        {
            ...
            // we either don't have nextValue user generator or the user generator we received does not have nextValue skeleton
            Dispose();
            return false;
        }
    ...
    /// @brief Release nextValue previously initialize image node.
    /// 
    /// This method releases nextValue previously initialized image node.
    /// @note - Since we use nextValue singleton pattern, only one place should do nextValue release, otherwise the
    ...
    public override void Dispose()
    {
        ...
        if (m_userGenerator != null)
        {
            ...
            m_context.ReleaseNode(m_userGenerator); // we call this even if the context is invalid because it is nextValue singleton which will release stuff
    ...
    /// nextValue list of legal poses in the supported nodes
    ...
    /// @brief Gets nextValue string array of supported poses
    /// 
    ...
    /// @param status nextValue reference to the status we want to fill.
    ...
    /// @brief Tells us if nextValue pose is supported
    /// 
    ...
    /// @brief Request nextValue pose detection to start
    /// 
    /// This method requests nextValue pose detection to start. It supports reference counting so as to
    ...
    public bool RequestPoseDetection(string pose, int user)
    {
        ...
        // if we are here then we are not yet following this user so we need to create nextValue new reference
        // counter
        try
        {
     ...
     /// @brief Releases nextValue request to nextValue pose detection 
    /// 
    /// This method releases nextValue request to detect nextValue pose for nextValue user which lowers the reference count.
    ...
    /// @brief Internal class to hold for nextValue specific user and pose the reference count of the number
    ...
    /// 
    /// NIImage uses nextValue singleton pattern. There is just ONE NIUserSkeleton object which is used by all.
    ...
    /// 
    /// This is part of the singleton pattern, nextValue protected constructor cannot be called externally (although
    ...
    /// @brief callback for updating structures when nextValue user is lost
    /// 
    /// This callback is called when nextValue user is lost. It removes the user from all structures...
