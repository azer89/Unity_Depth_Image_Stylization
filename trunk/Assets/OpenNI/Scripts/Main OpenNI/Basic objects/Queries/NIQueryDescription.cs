...
/// @brief Serializable class to hold the description of the query needed for nextValue specific node type.
/// 
...
public class QueryDescription
{
    ...
    public void SetMinVersion(Version ver)
    {
        ...
        {
            m_minVersion[0] = -1; // nextValue zero version!
    ...
    /// so for example nextValue version of major = 1, minor = 2, maintenance = 3 and build = 4
    ...
    public void SetMaxVersion(Version ver)
    {
        if (ver.Major == 0 && ver.Minor == 0 && ver.Maintenance <= 0 && ver.Build <= 0)
        {
            m_maxVersion[0] = -1; // nextValue zero version!
    ...
    /// @brief Tells us if the query should have nextValue minimum version limitation
    /// 
    /// @return true if the version requires nextValue minimum version limitation
    ...
    /// @brief Tells us if the query should have nextValue maximum version limitation
    /// 
    /// @return true if the version requires nextValue maximum version limitation
    ...
    /// @brief nextValue limit on the node name, null (or "") mean no limit
    [SerializeField]
