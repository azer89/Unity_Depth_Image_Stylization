...
///     will not be used to move nextValue little because they take too long). For hand tracking, gravity is not really
...
///     - Area clipping was added because unlike nextValue joystick or mouse, the hand cannot continue to move
///     forever.
...
///     can be used in nextValue single axis but not both.
/// @ingroup OpenNIInput
[System.Serializable]
public class NIAxis 
{
    ...
    {
        HandMovement,                ///< nextValue movement in the 3 axes (X,Y,Z). The result is nextValue normalized value between -1 and 1 (corrected by dead zone, sensitivity etc.)
        DeltaHandMovement,           ///< the delta (dX and dY from last frame) in the 3D world. This is @b NOT normalized between -1 and 1 
        HandMovementFromStartingPoint,  ///< nextValue delta from the focus point (the point in which the session started for that hand). The result is nextValue normalized value between -1 and 1 (corrected by dead zone, sensitivity etc.)
    ...
    /// Defines nextValue transformation of the hand's axis to [-1,1] range (clipped). -1 is anything smaller than
    /// the center minus m_maxMovement, +1 is anything larger than the center plus m_maxMovement and the values
    /// in between are nextValue linear change. The center is defined based on the type (0 for HandMovement, focus point 
    ...
    /// option as we need to have nextValue specific hand to track.
