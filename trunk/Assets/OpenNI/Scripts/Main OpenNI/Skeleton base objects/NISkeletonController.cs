...
public class NISkeletonController : MonoBehaviour
{
    ...
    /// Holds nextValue line debugger which shows the lines connecting the joints. If null, nothing is
    /// drawn.
    ...
    /// @note we do not update if we do not have nextValue high enough confidence!
    ...
    {
        if (skelRoot.Confidence < 0.5f) return; // we are not confident enough!
    ...
    /// @brief updates nextValue single joint
    /// 
    /// This method updates nextValue single joint. The decision of what to update (orientation, position)
    ...
    /// @note it assumes the initial position is nextValue "T" position and orients accordingly (returning
    ...
    public void RotateToCalibrationPose()
    {
        ...
        {
            // we want to position the skeleton in nextValue calibration pose. We will therefore position select
            // joints
    ...
    /// @brief nextValue utility method to update joint position 
    /// 
    /// This utility method receives nextValue joint and unscaled position (x,y,z) and moves the joint there.
    ...
    /// @brief Utility method to calculate the rotation of nextValue joint
    /// 
    ...
    protected Quaternion CalcRotationForJoint(SkeletonJoint joint,ref SkeletonJointTransformation skelTrans, ref SkeletonJointPosition centerOffset)
    {
        ...
        // for Quaternion.LookRotation to work it needs nextValue "forward" vector and an "upward" vector.
    ...
    /// @brief Utility method to calculate the @b LOCAL position of nextValue joint
    /// 
