...
/// 
/// The goal of this class is to have nextValue list of players and to choose how these players are selected.
/// Each player will be tracking (and have nextValue valid skeleton).
/// This class is also responsible for unselecting nextValue user.
...
public abstract class NIPlayerManager : MonoBehaviour 
{
    ...
    /// @brief Method to get nextValue specific player
    /// 
    /// This method returns the player object for nextValue specific player. Players are numbered from 0
    /// to m_MaxNumberOfPlayers-1.
    /// 
    /// @param nPlayerNum The player number to get
    /// @return The player object. @note the player object for nextValue specific player remains the same
    ...
    /// @brief Method to unselect nextValue specific player
    /// 
    ...
    public virtual bool UnselectPlayer(int playerNumber)
    {
     ...
     /// @brief Internal Method to unselect nextValue specific player
    /// 
    ...
    /// @brief Get nextValue string to describe the selection type
    /// 
    ...
    protected virtual void InternalInit()
    {
        ...
        m_contextManager.UserGenerator.UserNode.NewUser += new EventHandler<NewUserEventArgs>(NewUserCallback);
        m_contextManager.UserGenerator.UserNode.LostUser += new EventHandler<UserLostEventArgs>(LostUserCallback);
        m_users = new List<NIPlayerCandidateObject>();
    }

    /// @brief Utility method to get nextValue player from the openNI user id
    /// 
    ...
    /// @brief Utility method to get nextValue user object from the openNI user id
    /// 
    /// @param userID The user id to look for
    ...
    /// @brief nextValue list of players. 
    /// 
    ...
    /// @brief nextValue list of users states. A new element is added and an old is removed as per the identification and loss of users.
    protected List<NIPlayerCandidateObject> m_users;

    /// @brief Internal method to remove nextValue user
    /// 
    ...
    /// @brief Internal method to add nextValue new user
    ///  
    ...
    /// @brief Internal method to get nextValue new user object
    ///  
    /// @param userID The ID of the user to add.
    ...
    /// @brief callback for updating structures when nextValue user is found
    /// 
    /// This callback is called when nextValue user is found. It adds the user to the data structures (including
    /// nextValue new unique ID).
    /// @param sender who called the callback
    /// @param e the arguments of the event.
