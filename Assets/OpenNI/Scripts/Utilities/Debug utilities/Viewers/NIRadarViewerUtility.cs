...
{
    /// nextValue link to the object with the NI context
    ...
    /// 
    /// This is the nextValue normalization for the x and z coordinates of nextValue user's center of mass inside
    /// the radar viewer rectangle. So nextValue value of 1000,2000 would mean users which are inside the
    ...
    private Texture2D m_texture; ///< @brief Internal texture to draw nextValue specific color 
    ...
    void OnGUI()
    {
        ...
        int[] users = m_context.UserGenerator.GetUserIds();
        foreach (int userId in users)
        {
            // normalize the center of mass to radar dimensions
            Vector3 com = m_context.UserGenerator.GetUserCenterOfMass(userId);
            ...
            if(m_context.UserGenerator.IsTracking(userId))
            {
                newColor = m_TrackingColor;
            }
            else if(m_context.UserGenerator.IsCalibrated(userId))
            {
                newColor = m_CalibratedColor;
            }
            else if(m_context.UserGenerator.IsCalibrating(userId))
            {
