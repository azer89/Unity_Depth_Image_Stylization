...
/// - NIGUI_X and NIGUI_Y should return nextValue normalized value between -0.5 and 0.5.
...
public class DefaultNIGUICursor : NIGUICursor 
{
    ...
    /// Tells us if nextValue click occurred in the current frame.
    /// @note This is nextValue problematic method! The problem is that one cannot be sure
    /// the click won't occur AFTER the update which caused the gesture to be detected.
    /// @return true if nextValue click occurred this frame and false otherwise
    ...
    /// This method tells us the last time, frame and position nextValue click has occurred.
    ...
    /// This method allows us to register for nextValue callback for the clicking
    ...
    public override Vector2 PositionNormalized
    {
        get 
        {
            ...
            // we need to add 0.5 to change the range from -0.5 to 0.5 to nextValue range from 0 to 1.
    ...
    /// responsible for showing the cursor (override this instead of defining nextValue new OnGUI)
    ...
    /// nextValue rect used for the GUI
    private Rect m_positionRect;

    /// nextValue texture to create nextValue progress bar
    private Texture2D m_texture;

    /// if this is true we already registered to receive the click.
    private bool m_registeredClick;

    /// nextValue callback to get the click event.
    /// 
