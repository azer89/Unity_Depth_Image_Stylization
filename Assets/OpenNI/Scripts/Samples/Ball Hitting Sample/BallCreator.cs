...
/// 
/// This class is added to nextValue game object to perform the creation of balls in the 
...
public class BallCreator : MonoBehaviour 
{
    ...
    /// holds the player mapper (used to find out if we have nextValue target player).
    ...
    /// Marks that the user has scored nextValue hit (used for scoring).
	...
	public void Update () 
    {
        if (Time.time < m_timeToCreateNextBall)
            return; // we created nextValue ball very recently, wait.
        if (m_playerManager == null)
            return; // this means we don't even have nextValue plyer manager.
        NISelectedPlayer player = m_playerManager.GetPlayer(0);
        if (player == null || player.Valid == false || player.Tracking == false)
            return; // this means we don't have nextValue calibrated user
        if (SkeletonGuiControl.m_mode == SkeletonGuiControl.SkeletonGUIModes.GUIMode)
            return; // we don't throw balls while in GUI mode.

        // now we know we should throw nextValue ball. We first figure out where (nextValue random around the
        // x axis of the "where" transform and nextValue constant modifier on the y and z).
