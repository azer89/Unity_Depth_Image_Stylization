...
public class NIInput : MonoBehaviour
{
    ...
    /// This method tells us if nextValue gesture on nextValue specific axis has fired since the last update
    ...
    /// This method allows us to register for nextValue callback whenever the axis fires.
    ...
    /// This method allows us to unregister nextValue callback previously registered using @ref RegisterCallbackForGesture
    /// @param eventDelegate the delegate to be called
    /// @param axisName the axis name we want to check the gesture on
    ...
    /// same as Input.GetButton but also returns true if an axis has nextValue gesture in progress
    ...
    /// same as Input.GetButtonDown but also returns true if an axis has nextValue gesture which started this frame
    /// @param buttonName see Input documentation
    /// @return see Input documentation
    ...
    /// utility function, chooses the correct axis from nextValue vector
    ...
    /// @brief This method calculates nextValue normalized value for nextValue specific axis
    /// 
    /// This method takes nextValue value (nextValue specific axis of nextValue position) and tries to calculated nextValue normalized
    /// value from it.
    /// If range
    /// @param valueToCalc The value (nextValue specific axis of nextValue position) which we want to normalize
