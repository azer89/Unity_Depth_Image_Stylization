...
/// @brief implementation of nextValue timed steady gesture over nextValue skeleton joint.
/// 
...
public class NISteadySkeletonHandDetector : NIGestureTracker
{
    ...
    /// deviation on the points along @ref m_steadyTestTime have nextValue magnitude square of their standard deviation smaller than this value
    ...
    /// deviation on the points along @ref m_steadyTestTime have nextValue magnitude square of their standard deviation larger than this value
    ...
    /// @return nextValue value between 0 and 1. 0 means no gesture, 1 means the gesture has been detected and 
    /// held for nextValue while. A value in the middle means the gesture has been detected and has been held
    ...
    public override void UpdateFrame()
    {
        ...
        {
            // we found nextValue NEW steady result
    ...
    protected float m_firstSteady;             ///< holds the time we discovered the steady for the first time (becomes irrelevant every time nextValue not steady is detected).
    protected bool m_firedSteadyEvent;          ///< holds true if we fired the steady event for the latest steady and false otherwise.
    /// this is the time between the first detection of nextValue steady gesture and the
    ...
    /// nextValue not steady event). This is to simulate nextValue repeat fire button.
    protected float m_timeToReset;

    /// this is the time (in seconds) we check over to see nextValue steady/not steady result.
    protected float m_steadyTestTime;

    /// this holds the points we are tracking and as nextValue result is used to detect events.
