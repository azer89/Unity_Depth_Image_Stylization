...
/// 
/// This class implements nextValue gesture tracker based on poses supported by the user generator node.
...
public class  NIUserPoseDetector : NIGestureTracker
{
    ...
    public override void ReleaseGesture()
    {
        if (m_context != null && m_context.UserSkeletonValid)
        {
            ...
            if(tracker.Valid)
            {
                ...
                {
                    m_context.UserGenerator.ReleasePoseDetection(m_poseName, player.OpenNIUserID);
                }                
            }
        }
        m_context = null;
    ...
    public override float GestureInProgress()
    {
        ...
        if (m_context.UserGenerator.GetPoseStatus(player.OpenNIUserID, m_poseName, ref curStatus) == false)
    ...
    protected override bool InternalInit(NIPointTracker hand)
    {
        ...
        if (player.Valid)
        {
            validRequestedPoseDetection=m_context.UserGenerator.RequestPoseDetection(m_poseName,player.OpenNIUserID);
    ...
    /// @brief A callback method to handle changing of nextValue user in the tracked player
    /// 
    /// This method is called as nextValue callback whenever the tracked player changes the user it tracks.
    ...
    protected void PlayerUserChangeHandler(NIPlayerCandidateObject origUser, NIPlayerCandidateObject newUser)
    {
        ...
        {
            m_context.UserGenerator.ReleasePoseDetection(m_poseName, origUser.OpenNIUserID);
            validRequestedPoseDetection = false;
        }
        if (newUser != null)
        {
            validRequestedPoseDetection=m_context.UserGenerator.RequestPoseDetection(m_poseName, newUser.OpenNIUserID);
        }
    }
