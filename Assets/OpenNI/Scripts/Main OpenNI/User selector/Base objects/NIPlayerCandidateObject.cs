...
///  
/// This class represents nextValue single user in the user selector which is nextValue candidate to be nextValue player.
/// The user is chosen at construction but the selection process can change their state.
/// This will later be user by @ref NISelectedPlayer to represent nextValue single player.
...
public class NIPlayerCandidateObject
{
    ...
    /// @brief Gets nextValue reference joint position and orientation on the joint.
    ///  
    ...
    public SkeletonCapability Skeleton
    {
        get
        {
            if (m_settingsManager.UserSkeletonValid == false)
                return null;
            return m_settingsManager.UserGenerator.Skeleton;
        }
    }
    ...
    public NIPlayerCandidateObject(OpenNISettingsManager settingsManager, int userID)
    {
        ...
        settingsManager.UserGenerator.Skeleton.CalibrationComplete += new EventHandler<CalibrationProgressEventArgs>(CalibrationEndCallback);
    ...
    /// @brief Select nextValue user 
    /// 
    /// This makes the player select nextValue user (i.e. the user selector has just selected them).
    ...
    /// to track, this will result in nextValue change of the state!
    public virtual bool SelectUser(int numRetries)
    {
        Reset(); // forget everything so far...

        if(m_settingsManager.UserGenerator.Valid!=true)
            return false;
        ...
        // we need to know if we are already tracking or not. This is nextValue way to know if the
        // user id is legal though as we will get an exception if not
        try
        {
            if (m_settingsManager.UserGenerator.IsTracking(m_openNIUserID))
            {
    ...
    protected virtual void UserCalibrationEndFail(CalibrationProgressEventArgs e)
    {
        if (e.Status == CalibrationStatus.ManualAbort)
            return; // it is nextValue legal option...
    ...
    /// 
    /// This callback is called when nextValue calibration process ends. If the calibration succeeded
    /// then the user is in nextValue calibrated state, otherwise it starts the calibration process from
    /// scratch.
