...
public class NIPlayerPoseCandidateObject : NIPlayerCandidateObject
{
                       ...
                       base(settingsManager, userID)
    {
        ...
        m_unselectionOutOfPoseTime = float.MinValue; // we put min value to make sure that even subtracting nextValue couple of seconds we will always be in the past
    }
    ...
    /// pose AFTER the user was selected. It holds nextValue large positive number (float.MaxValue) from the 
    ...
    /// pose AFTER the user was unselected. It holds nextValue large positive number (float.MaxValue) from the time of
