...
{
    public float m_maxAllowedDistance = 100000.0f; ///< @brief defines nextValue maximum distance, we can't have depth farther than this.
    public float m_failurePenalty = 10000.0f; ///< @brief Defines the penalty (in mm) for users who failed to track and have not moved.
    public float m_minCOMChangeForFailure=200; ///< @brief The minimum change in COM needed to retry nextValue failure
    
    /* The lag in variable property of nextValue system with the respect 
    ...
    protected virtual void UpdateUserPriority(NIPlayerCandidateObject user)
    {
        ...
        if (userCom == null)
            return; // irrelevant
        Vector3 com=m_contextManager.UserGenerator.GetUserCenterOfMass(userCom.OpenNIUserID);
