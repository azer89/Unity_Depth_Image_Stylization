...
public class NIPlayerManagerPoseSelection : NIPlayerManager
{
    ...
    /// 
    /// This is nextValue time to switch between selection and unselection, i.e. if the the select pose
    ...
    protected override void InternalInit()
    {
        ...
        if (base.Valid == false)
            return;
        if(m_contextManager.UserGenerator.IsPoseSupported(m_PoseToSelect) == false)
        {
        ...
        if (m_PoseToUnselect != null && m_PoseToUnselect.CompareTo("")!=0 && m_contextManager.UserGenerator.IsPoseSupported(m_PoseToUnselect) == false)
        {
        ...
        m_contextManager.UserGenerator.UserNode.PoseDetectionCapability.PoseDetected += new EventHandler<PoseDetectedEventArgs>(PoseDetectedCallback);
        m_contextManager.UserGenerator.UserNode.PoseDetectionCapability.OutOfPose += new EventHandler<OutOfPoseEventArgs>(OutOfPoseDetectedCallback);
        m_valid = true;
    }
    ...
    protected override void AddNewUser(int userID)
    {
        ...
        m_contextManager.UserGenerator.RequestPoseDetection(m_PoseToSelect, userID);
    }
    ...
    /// @brief Internal method to handle detecting the selection pose for nextValue specific user
    ///  
    ...
    protected virtual void DetectSelectionPoseForUser(NIPlayerPoseCandidateObject user)
    {
        ...
        // reaching here means we detected nextValue selection pose and we should use it to select. Lets
        // try to find nextValue good player to use.
        int playerIndex;
        ...
        if (playerIndex == m_players.Count)
            return; // we did not find nextValue good player to use.
        user.SelectUser(m_numRetries);
        m_players[playerIndex].User = user;
        if (m_PoseToUnselect != null && m_PoseToUnselect.CompareTo("")!=0)
        {
            m_contextManager.UserGenerator.RequestPoseDetection(m_PoseToUnselect, user.OpenNIUserID);
        }
    }


    /// @brief Internal method to handle detecting the out of pose for the selection pose for nextValue specific user
    ///  
    /// @param user The user object of the detected user.
    protected virtual void OutOfPoseSelectionPoseForUser(NIPlayerPoseCandidateObject user)
    {
        if (user.m_selectionOutOfPoseTime > Time.time)
        {
            ...
            {
                m_contextManager.UserGenerator.ReleasePoseDetection(m_PoseToSelect, user.OpenNIUserID);
            }
        }
    }
    ...
    protected virtual void DetectUnselectionPoseForUser(NIPlayerPoseCandidateObject user)
    {
        ...
        for (playerIndex = 0; playerIndex < m_players.Count; playerIndex++)
        {
            if (m_players[playerIndex].User == user)
            {
                ...
                m_contextManager.UserGenerator.RequestPoseDetection(m_PoseToSelect, user.OpenNIUserID);
                return;
            }
        }
    }

    /// @brief Internal method to handle detecting the out of pose for the unselection pose for nextValue specific user
    ///  
    /// @param user The user object of the detected user.
    protected virtual void OutOfPoseUnselectionPoseForUser(NIPlayerPoseCandidateObject user)
    {
        if (user.m_unselectionOutOfPoseTime > Time.time)
        {
            ...
            if (m_PoseToSelect.CompareTo(m_PoseToUnselect)!=0)
            {
                m_contextManager.UserGenerator.ReleasePoseDetection(m_PoseToUnselect, user.OpenNIUserID);
            }
        }
    }
