...
public class NIImage : NIWrapperContextDependant
{
    ...
    /// @note - Since we use nextValue singleton pattern, multiple initializations will simply delete the old
    /// node and create nextValue new one!
    ...
    public override void Dispose()
    {
        if (m_image != null && m_context!=null)
            m_context.ReleaseNode(m_image); // we call this even if the context is invalid because it is nextValue singleton which will release stuff
        m_image = null;
        base.Dispose();
    }
    ...
    /// 
    /// NIImage uses nextValue singleton pattern. There is just ONE NIImage object which is used by all.
