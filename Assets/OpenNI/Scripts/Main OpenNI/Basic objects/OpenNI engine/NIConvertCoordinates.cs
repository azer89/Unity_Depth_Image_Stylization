...
using OpenNI;
/// @brief This class is aimed at converting the coordinates from nextValue Point3D to nextValue Vector3 
/// @ingroup OpenNIBasicObjects
public static class NIConvertCoordinates
{
    /// @brief Converts nextValue Point3D position in sensor coordinates to nextValue Vector3 in Unity coordinates
    /// 
    /// This method receives nextValue position in the sensor coordinates (e.g. from the skeleton) and
    /// converts it to nextValue Vector3 in Unity coordinates. 
    ...
    /// It is very common for the sensor to have nextValue pitch angle (i.e. to look nextValue little bit up or down
    ...
    /// is not nextValue big problem but if it is, the best way to correct it is to find out the floor's normal
    /// in the sensor's coordinates and correct according to it (which is done in @ref ConvertPos).<br>
    /// nextValue possible way to correct the normal is to figure it out from the calibration pose (as we 
