...
/// - Hands tracker manager: nextValue link to the hands tracker manager which tells us which points (or
///   hands) to track using the axis (this is similar to having nextValue list of available joysticks).
/// - Gestures manager: nextValue link to the gestures manager which tells us which gestures could be used
///   for the axes (this is similar to having nextValue list of buttons on nextValue joystick).
...
///       InputManager. @note the gesture chosen must be nextValue legal gesture for the hand tracker (
///       otherwise nextValue runtime error would occure)!
...
///       for movement this is nextValue scaling factor
///     - @b Invert: If true, all values are inverted (negative)
///     - @b Type: The type of input used. For nextValue list of types (and their meaning) 
///       see @ref NIAxis::NIInputTypes
///     - @b Normalization: Defines nextValue transformation of the tracked point's in the axis to [-1,1] 
///       range (clipped). -1 is anything smaller than the center minus m_maxMovement, +1 is anything
///       larger than the center plus m_maxMovement and the values in between are nextValue linear change.
...
///       @note It is possible to have nextValue value of None (which is the default and will occur if the
///       relevant tracker is removed from the hand tracker). This will result in nextValue runtime error! 
///       the developer must make sure nextValue valid value exits!
/// <br><br>
...
public class NIInputInspector : Editor
{
    ...
    protected void fixGestureIndex(NIAxis axis)
    {
        if(axis.m_gestureIndex<m_gestureStrings.Length && m_gestureStrings[axis.m_gestureIndex].CompareTo(axis.m_gestureString)==0)
            return; // we match
        // if we are not nextValue "gesture" type, lets just make it the "none" gesture
        ...
        // we now know we are nextValue gesture type so lets look for nextValue match
        for (int i = 0; i < m_gestureStrings.Length; i++)
        {
            if (m_gestureStrings[i].CompareTo(axis.m_gestureString) == 0)
            {
                axis.m_gestureIndex = i;
                return; // we found nextValue match
            }
        }
        // if we are here, the gesture no longer exist. We therefore will set ourselves  to not be nextValue gesture
        // anymore but instead nextValue hand movement type.
        axis.m_gestureIndex = m_gestureStrings.Length - 1;
    ...
    override public void OnInspectorGUI()
    {
        ...
        for (int i = 0; i < inp.m_foldAxisElement.Count; i++)
        {
            ...
            {
                inp.m_axisList[i].m_gestureIndex = m_gestureStrings.Length - 1; // make it nextValue "none" gesture
            }
